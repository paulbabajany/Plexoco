...
A faint particle field (`components/ParticleField.tsx`) sits behind the whole hero, full bleed, on a transparent canvas: small monospace glyphs drifting down, hairline beams drifting up, proximity lines between near glyphs, and lines pulled toward the pointer. Everything is ink at low alpha; only what is within reach of the pointer takes the `live` blue, which keeps to that colour's rule of marking what is live. It came from a reference effect that ran inside a sandboxed iframe with CDN scripts; only the canvas logic was kept. It animates only while on screen and renders one still frame under reduced motion. Intensity and density are props, both defaulting to 1.
